...
4. **A person sends every message.** The console sends one email at a time from the
   operator's own mailbox, or one text from their Quo number, after the operator
   has read that message and pressed Send on it. Nothing sends on a schedule, in bulk, or from a job, and nothing
...
- Brand tokens: asphalt `#16120E`, chalk `#ECE6DC`, safety orange `#F25C1F`.
  PT Sans display, Work Sans body. 16px minimum, mobile first.
